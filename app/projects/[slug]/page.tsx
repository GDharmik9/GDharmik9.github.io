  ...
  return (
    <main className="min-h-screen bg-slate-50 dark:bg-ink">
      ...
      <section className="relative overflow-hidden bg-[#050816] px-6 pt-32 text-white lg:px-8">
        <div
          className={`absolute inset-0 bg-gradient-to-br ${project.gradient} opacity-25 blur-3xl`}
        />
            ...
            <GlassCard className="bg-white/10 text-white">
              ...
              <div className="mt-6 flex flex-wrap gap-2">
                {project.stack.map((tech) => (
                  <span
                    key={tech}
                    className="rounded-full bg-white/10 px-3 py-1 text-xs font-bold text-cyan-100"
                  >
                    {tech}
                  </span>
                ))}
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-6 py-16 lg:grid-cols-[.7fr_.3fr] lg:px-8">
        ...
        <aside className="space-y-6">
          ...
          <GlassCard>
            ...
            <div
              className={`mt-5 rounded-3xl bg-gradient-to-br ${project.gradient} p-5 text-white`}
            >
              <div className="rounded-2xl border border-white/20 bg-white/15 p-5 backdrop-blur">
                <p className="text-sm opacity-80">Visual placeholder</p>
                <p className="mt-8 text-2xl font-black">{project.name}</p>
...
function CaseBlock({
  ...
  return (
    <GlassCard>
      ...
      <div className="leading-7 text-slate-600 dark:text-slate-300">
        {children}
      </div>
    </GlassCard>
  );
