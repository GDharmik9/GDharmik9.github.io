  ...
  return (
    <Link
      ...
      className={cn(
        ...
        variant === "primary" && "bg-white text-slate-950 shadow-glow hover:-translate-y-0.5 hover:bg-cyan-50",
        variant === "secondary" && "border border-white/15 bg-white/10 text-white backdrop-blur hover:bg-white/15",
  ...
  return (
    <section id={id} className="mx-auto w-full max-w-7xl px-6 py-20 lg:px-8">
      <div className="mb-10 max-w-3xl">
        <p className="mb-3 text-sm font-bold uppercase tracking-[0.3em] text-cyan-500">{eyebrow}</p>
        <h2 className="text-3xl font-bold tracking-tight text-slate-950 dark:text-white md:text-5xl">{title}</h2>
      </div>
      {children}
    </section>
  );
...
export function GlassCard({ children, className }: { children: React.ReactNode; className?: string }) {
  return <div className={cn("rounded-3xl border border-slate-200/70 bg-white/75 p-6 shadow-2xl shadow-slate-900/5 backdrop-blur-xl dark:border-white/10 dark:bg-white/[0.06] dark:shadow-black/20", className)}>{children}</div>;
}
